...
from .bounds import ellipsoid
from .basic_sampler import basic_sampler


class ellipsoid_sampler(basic_sampler):
    """ Nested sampling implementing a single ellipsoid boundary. This 
    involves drawing samples from an N-dimensional ellipsoid around the
    region occupied  by the live points, expanded by some factor.
    ...
    expansion : float
        The factor by which the volume of the ellipsoid is expanded. The
    ...
    def update_bound(self):
        ...
        if not self.n_samples % self.update_interval:
            self.bound = ellipsoid(self.live_cubes, expansion=self.expansion) 
